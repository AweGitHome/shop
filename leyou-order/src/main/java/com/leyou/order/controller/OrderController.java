...
public class OrderController {
    ...
    public ResponseEntity<Map> getParamMap(){
        Map<Long,String> paramMap = new HashMap<>();
        List<SpecParam> params = specClient.queryParams(null, null, null, null);
        params.forEach(param-> paramMap.put(param.getId(),param.getName()));
        return ResponseEntity.ok(paramMap);
    }

    @GetMapping("getAddressList")
    public ResponseEntity<List<Address>> getAddList(){
        Long id = LoginInterceptor.getLoginUser().getId();
        List<Address> addressList = this.userClient.getAddressList(id);
